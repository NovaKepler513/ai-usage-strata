...
- 可选“参考期”，可保存一段需要反复回看的审计期、冲刺期或报告期；它和“全部／本月／近 4 周”并列。
- 可勾选工作分类；选中后会显示该分类在所选时间里的约分配值，并重绘周／月条带。
- 中英文界面切换；选择会保存在这台设备上。
...
1. 下载或克隆仓库。
2. macOS 可双击 `启动·AI Usage Strata.command`；它会只在本机启动 `127.0.0.1:8770` 并打开页面。其他系统用任意静态文件服务器打开这个目录即可。
...
欢迎改进 bug、无障碍、翻译、导入适配器、示例账本和文档。提交前先看 [CONTRIBUTING.md](CONTRIBUTING.md) 与 [SECURITY.md](SECURITY.md)。涉及较大功能变化时，先开 Issue 讨论，避免破坏“本机优先、数据不上云”的边界。
